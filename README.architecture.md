...
`dart compile js` is not produced. The template targets Wasm only, since
dart2js' interop has notable runtime quirks (`Future.toJS`, `List.toJS`)
that the Wasm path does not exhibit and that a template should not have
to apologise for.
...
[`compat.ts`](../../ggsuite/gg_dart_js_bridge_template/typescript/compat.ts),
